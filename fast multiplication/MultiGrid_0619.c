...
int main()
{
	...
	{
		Multigrid_Iteration(U, F, N);		// 每做一次就，
		r1 = Residual(R, U, F, N);			// 計算殘差量 
		k ++;
		printf("%d:%e %f\n",k,r1,r1/r);
		r = r1;
	}
...
int GaussSeidel_Iteration(double **U, double **F, int N)
{
	...
	/*   //逆向跑，多跑幾次，先 MARK 起來才看得出 GaussSeidel_Iteration 在做啥。 
	for(i=0;i<N-1;++i) 
	{
		for(j=N-2;j>=0;j--)
		{
...
int Multigrid_Iteration(double **U, double **F, int N)
{
	...
	else
	{
		...
		R[0] = (double *) malloc( (N-1)*(N-1)*sizeof(double) );
		for(i=1;i<N-1;++i) R[i] = R[i-1] + (N-1);		
		Un = (double **) malloc( (N/2-1)*sizeof(double*) );					// U => E
		Un[0] = (double *) malloc( (N/2-1)*(N/2-1)*sizeof(double) );
		for(i=1;i<N/2-1;++i) Un[i] = Un[i-1] + (N/2-1);	
		Fn = (double **) malloc( (N/2-1)*sizeof(double*) );					// F => R
		...
		for(i=0;i<N/2-1;++i)
		{
			for(j=0;j<N/2-1;++j)			// 投影 
			{
				Un[i][j] = 0.0;
				Fn[i][j] = 4*R[2*i+1][2*j+1];								// 幾何方式 
				/*
				Fn[i][j] = ((R[2*i][2*j]+2*R[2*i][2*j+1]+R[2*i][2*j+2])+
				           (2*R[2*i+1][2*j]+4*R[2*i+1][2*j+1]+2*R[2*i+1][2*j+2])+
						   (R[2*i+2][2*j]+2*R[2*i+2][2*j+1]+R[2*i+2][2*j+2]))/4.0;	//代數方式 ， 可試試看哪個好。 
				*/
			}
		}
...
double Residual(double **R, double **U, double **F, int N)
{
	...
	for(i=0;i<N-1;++i) 
	{
		for(j=0;j<N-1;++j)
		{
			s = F[i][j]-U[i][j]*(-4);		// R[i][j]=F[i][j]-(-4)*U[i][j]-U[i][j-1]-U[i][j+1]-U[i-1][j]-U[i+1][j]
			if(j-1>=0) s -= U[i][j-1];
			if(j+1< N-1) s -= U[i][j+1];
			if(i-1>=0) s -= U[i-1][j];
			if(i+1< N-1) s -= U[i+1][j];
			if(fabs(s) > r) r = fabs(s); 	// r = max_{i,j}|R[i][j]|
			R[i][j] = s;
		}
	}
	return r;
